...
export class DedustV2Simulation extends Simulation {
    ...
    private sendNativeSwap(sender: SandboxContract<TreasuryContract>, amount: bigint) {
        ...
        const body = beginCell()
            ...
            .storeUint(0, 64)
            .storeCoins(amount)
            .storeRef(DedustV2Simulation.buildSwapPayload(0n, this.getDeadline(60000)))
    ...
    private getDeadline(expireIn: number) {
        const now = this.chain.now ?? Math.floor(Date.now() / 1000)
        return now + expireIn
    }

    private static buildSwapPayload(minAmountOut: bigint, deadline: number) {
